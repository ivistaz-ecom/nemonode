const empmodeofentry = require("../../models/empmodeofentry");
const bcrypt = require("bcrypt");
const sequelize = require("../../util/database");
const { Op } = require("sequelize");
const { base64decode } = require("nodejs-base64");
const moduleName = 'Mode Of Entry'
...
const list = async (req, res) => {
  try {
    ...
    let where = `modeID IS NOT NULL`;
    if(name!=="") {
      where+=` AND modeName LIKE '%${name}%'`;
    }
    if(status!=="") {
      where+=` AND modeStatus='${status}'`;
    }
    let countWhere = {
      modeID: { [Op.ne]: null }
    };
    ...
    let query = `SELECT modeID, modeName, modeStatus FROM empmodeofentry WHERE ${where} ORDER BY modeID DESC LIMIT ${
      page - 1
    }, ${limit} `;
    ...
    if (listData.length > 0) {
      let totalRecords = await empmodeofentry.count(countWhere);
...
const save = async (req, res) => {
  try {
    ...
    try {
      const newUser = await empmodeofentry.create({
        modeName,
        modeStatus,
        modeCreatedBy:employeeID,
        modeCreatedOn:new Date()
      });
      if(newUser!=="") {
...
const getDetails = async (req, res) => {
  try {
    ...
    let where = `modeID='${ID}'`;
    let query = `SELECT * FROM empmodeofentry WHERE ${where}`;
...
const update = async (req, res) => {
  try {
    ...
    const getDetail = await empmodeofentry.findOne({ where: { modeID: ID } });
    if(getDetail!==null) {
      const updateData = {
        modeName:request?.modeName ?? '',
        modeStatus:request?.modeStatus ?? '',
        modeUpdatedBy:employeeID,
        modeUpdatedOn:new Date()
      }

      await empmodeofentry.update(updateData, {
        where: { modeID: ID },
      });
...
const deleteData = async (req, res) => {
  try {
    ...
    const getDetail = await empmodeofentry.findOne({ where: { modeID: ID } });
    if(getDetail!==null) {
      const deleteDetails = await empmodeofentry.destroy({ where: { modeID: ID } });
...
const dropdownlist = async (req, res) => {
  try {
    let query = `SELECT modeID AS value, modeName AS label FROM empmodeofentry WHERE modeStatus='A' `;
    const listData = await sequelize.query(query, {
      type: sequelize.QueryTypes.SELECT,
    });
    if (listData.length > 0) {
