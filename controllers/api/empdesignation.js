...
const list = async (req, res) => {
  try {
    ...
    let where = `designationID IS NOT NULL`;
    if(name!=="") {
      where+=` AND designationName LIKE '%${name}%'`;
    }
    if(status!=="") {
      where+=` AND designationStatus='${status}'`;
    }
    let countWhere = {
    ...
    let query = `SELECT designationID, designationName, designationStatus FROM empdesignation WHERE ${where} ORDER BY designationID DESC LIMIT ${
      page - 1
    }, ${limit} `;
    ...
    if (listData.length > 0) {
      let totalRecords = await empdesignation.count(countWhere);
...
const save = async (req, res) => {
  try {
    ...
    try {
      const newUser = await empdesignation.create({
        designationName,
        designationStatus,
        designationCreatedBy:employeeID,
        designationCreatedOn:new Date()
      });
      if(newUser!=="") {
...
const getDetails = async (req, res) => {
  try {
    ...
    let where = `designationID='${ID}'`;
    let query = `SELECT * FROM empdesignation WHERE ${where}`;
...
const update = async (req, res) => {
  try {
    ...
    const getDetail = await empdesignation.findOne({ where: { designationID: ID } });
    if(getDetail!==null) {
      const updateData = {
        designationName:request?.designationName ?? '',
        designationStatus:request?.designationStatus ?? '',
        designationUpdatedBy:employeeID,
        designationUpdatedOn:new Date()
      }

      await empdesignation.update(updateData, {
...
const deleteData = async (req, res) => {
  try {
    ...
    const getDetail = await empdesignation.findOne({ where: { designationID: ID } });
    if(getDetail!==null) {
      const deleteDetails = await empdesignation.destroy({ where: { designationID: ID } });
...
const dropdownlist = async (req, res) => {
  try {
    let query = `SELECT designationID AS value, designationName AS label FROM empdesignation WHERE designationStatus='A' `;
    const listData = await sequelize.query(query, {
      type: sequelize.QueryTypes.SELECT,
    });
    if (listData.length > 0) {
