const empcity = require("../../models/empcity");
...
const list = async (req, res) => {
  try {
    ...
    let where = `cityID IS NOT NULL`;
    if(name!=="") {
      where+=` AND cityName LIKE '%${name}%'`;
    }
    if(status!=="") {
      where+=` AND cityStatus='${status}'`;
    }
    let countWhere = {
      cityID: { [Op.ne]: null }
    };
    ...
    let query = `SELECT cityID, cityName, cityStatus FROM empcity WHERE ${where} ORDER BY cityID DESC LIMIT ${
      page - 1
    }, ${limit} `;
    ...
    if (listData.length > 0) {
      let totalRecords = await empcity.count(countWhere);
...
const save = async (req, res) => {
  try {
    ...
    try {
      const newUser = await empcity.create({
        cityName,
        cityStatus,
        cityCreatedBy:employeeID,
        cityCreatedOn:new Date()
      });
      if(newUser!=="") {
...
const getDetails = async (req, res) => {
  try {
    ...
    let where = `cityID='${ID}'`;
    let query = `SELECT * FROM empcity WHERE ${where}`;
...
const update = async (req, res) => {
  try {
    ...
    const getDetail = await empcity.findOne({ where: { cityID: ID } });
    if(getDetail!==null) {
      const updateData = {
        cityName:request?.cityName ?? '',
        cityStatus:request?.cityStatus ?? '',
        cityUpdatedBy:employeeID,
        cityUpdatedOn:new Date()
      }

      await empcity.update(updateData, {
        where: { cityID: ID },
      });
...
const deleteData = async (req, res) => {
  try {
    ...
    const getDetail = await empcity.findOne({ where: { cityID: ID } });
    if(getDetail!==null) {
      const deleteDetails = await empcity.destroy({ where: { cityID: ID } });
...
const dropdownlist = async (req, res) => {
  try {
    let query = `SELECT cityID AS value, cityName AS label FROM empcity WHERE cityStatus='A' `;
    const listData = await sequelize.query(query, {
      type: sequelize.QueryTypes.SELECT,
    });
    if (listData.length > 0) {
