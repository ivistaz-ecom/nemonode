...
async function displayPort(page = 1, limit = 10) {
    try {
        ...
        const portResponse = await axios.get(`http://nemonode.ivistaz.co:8000/others/view-port?page=${page}&limit=${limit}`, { headers: { "Authorization": token } });
...
async function deletePort(portId, event) {
    ...
    const id = portId;
    const url = `http://nemonode.ivistaz.co:8000/others/delete-port/${id}`;
