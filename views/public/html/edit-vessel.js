...
async function displayVessels(page = 1, limit = 10) {
    try {
        ...
        const vesselResponse = await axios.get(`http://nemonode.ivistaz.co/others/view-vessels?page=${page}&limit=${limit}`, { headers: { "Authorization": token } });
...
async function deleteVessel(vesselId, event) {
    ...
    const url = `http://nemonode.ivistaz.co/others/delete-vessels/${vesselId}`;
...
async function displayVesselTypes(page = 1, limit = 10) {
    try {
        ...
        const vslTypeResponse = await axios.get(`http://nemonode.ivistaz.co/others/view-vsl?page=${page}&limit=${limit}`, { headers: { "Authorization": token } });
...
async function deleteVesselType(vesselTypeId, event) {
    ...
    const url = `http://nemonode.ivistaz.co/others/delete-vsl/${vesselTypeId}`;
