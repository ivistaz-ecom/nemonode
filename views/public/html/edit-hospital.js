...
async function displayHospital(page = 1, limit = 10) {
    try {
        ...
        const hospitalResponse = await axios.get(`http://localhost:8001/others/view-hospital?page=${page}&limit=${limit}`, { headers: { "Authorization": token } });
...
async function deleteHospital(hospitalId, event) {
    ...
    const id = hospitalId;
    const url = `http://localhost:8001/others/delete-hospital/${id}`;
