...
async function fetchAndDisplayDropdowns() {
    try {
        ...
        const portsResponse = await axios.get("http://nemonode.ivistaz.co/others/view-port", { headers: { "Authorization": token } });
...
function formatDate(dateString) {
  ...
  document.getElementById('contractForm').addEventListener('submit', async function (event) {
    ...
    try {
        ...
        const response = await axios.put(`http://nemonode.ivistaz.co/candidate/update-contract-details/${contractId}`, formData, config);
