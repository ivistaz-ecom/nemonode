...
const editContract = async (candidateId,id, rank, company, vslName, vesselType, sign_on_port, sign_on, wage_start, eoc, wages, currency, wages_types, sign_off, sign_off_port, reason_for_sign_off, aoa_number, emigrate_number, documents, aoa, created_by, event) => {
...
// Example obfuscated ID from the URL
...
async function handleContractForm(event) {
    ...
    const candidateId = decodedId
    const created_by = decodedToken.userId;

    // Retrieve values from form fields
    const rank = document.getElementById('candidate_c_rank').value.trim();
    const company = document.getElementById('candidate_vendor').value.trim();
    const vslName = document.getElementById('vesselDropdown').value.trim();
...
async function createCompanyDropdown() {
    ...
    const token = localStorage.getItem('token');
    const decodedToken = decodeToken(token); // Assuming decodeToken is a function that decodes the token
    console.log(decodedToken)
    // Extract userClient (company ID) value from the decoded token
    const userClient = decodedToken.userClient; // This should be the company ID
    ...
    try {
        ...
        // Assuming response data contains the company details
        ...
        // Add the company from the response
        const option = document.createElement('option');
        option.value = companyDetails.company_id; // Set company_id as the value
        option.text = companyDetails.company_name; // Set company_name as the display text
        companyDropdown.appendChild(option);

    } catch (error) {
        console.error('Error fetching company details:', error);
        // Handle error if the company details could not be fetched
    }
}
