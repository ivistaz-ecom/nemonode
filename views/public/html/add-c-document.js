    ...
    try {
        ...
        documentDetails.forEach(doc => {
            ...
            row.innerHTML = `
                ...
                <td><a href='${config.APIURL}views/public/files/${doc.document_files}' target="_blank">Click here to view Document!</a></td>
...
async function deleteDocument(documentId) {
    ...
    if (confirmDelete) {
        try {
            ...
            const response = await axios.delete(`${config.APIURL}candidate/document-delete/${documentId}`,{
...
document.getElementById('documentForm').addEventListener('submit', async function (event) {
    ...
    try {
        const response = await axios.post(`${config.APIURL}candidate/document-details/${id}`, formData, {
            headers: {
                'Authorization': token,
                'Content-Type': 'application/json'
            }
...
async function fetchDocumentTypes() {
    try {
        const response = await axios.get(`${config.APIURL}others/get-documenttype`);
