...
document.getElementById("vessel-form").addEventListener("submit", async (e) => {
    ...
    try {
        // Add a new vessel
        await axios.post("http://nemonode.ivistaz.co:8000/others/create-vessel", { vesselName }, { headers: { "Authorization": token } });
...
document.getElementById("vsl-form").addEventListener("submit", async (e) => {
    ...
    try {
        const serverResponse = await axios.post("http://nemonode.ivistaz.co:8000/others/create-vsl", {
            vesselName,
            vesselType,
            vsl_company,
            imoNumber,
