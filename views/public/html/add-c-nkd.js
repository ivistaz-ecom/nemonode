  ...
  document.getElementById('userManagementSections').style.display = 'block';
    ...
    document.getElementById('nkd_form').addEventListener('submit', async (e) => {
        try {
            ...
            const response = await axios.post(`http://nemonode.ivistaz.co:8000/candidate/kin-details/${memId}`, formData, { headers: { "Authorization": token } });
    ...
    const fetchAndDisplayNkdData = async () => {
        try {
            const memId = localStorage.getItem('memId');
            const response = await axios.get(`http://nemonode.ivistaz.co:8000/candidate/get-nkd-details/${memId}`, { headers: { "Authorization": token } });
