...
async function fetchAndDisplayVendors(page = 1, limit = 10) {
    try {
        ...
        const response = await axios.get(`http://localhost:8001/others/view-vendor?page=${page}&limit=${limit}`, {
...
document.getElementById('addVendorButton').addEventListener('click', async () => {
    try {
        ...
        const response = await axios.post('http://localhost:8001/others/create-vendor', formData,{headers:{"Authorization":token}});
...
async function deleteVendor(vendorId) {
    try {
        ...
        const response = await axios.delete(`http://localhost:8001/others/delete-vendor/${vendorId}`, {
            headers: { "Authorization": token }
        });
