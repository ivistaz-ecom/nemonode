...
async function handleContractForm(event) {
    ...
    try {
        const response = await axios.post(`http://nemonode.ivistaz.co/candidate/contract-details/${candidateId}`, contractDetails, {
            headers: {
                'Authorization': token,
                'Content-Type': 'application/json'
            }
...
async function fetchAndDisplayVesselType() {
    try {
        const serverResponse = await axios.get("http://nemonode.ivistaz.co/others/view-vessels", { headers: { "Authorization": token } });
