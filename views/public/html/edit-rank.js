...
async function displayRank(page = 1, limit = 10) {
    try {
        ...
        const rankResponse = await axios.get(`http://nemonode.ivistaz.co/others/view-rank?page=${page}&limit=${limit}`, { headers: { "Authorization": token } });
...
async function deleteRank(rankId, event) {
    ...
    const id = rankId;
    const url = `http://nemonode.ivistaz.co/others/delete-rank/${id}`;
