...
document.getElementById('search_btn').addEventListener('click', async function (e) {
  ...
  try {
      ...
      const response = await axios.post('http://nemonode.ivistaz.co/search', { 
          search: searchValue,
...
return decodedToken.userGroup
  ...
  const searchForm = document.getElementById('search-form');
    ...
    //   const documentNumber = document.getElementById('documentNumber').value;
      ...
      axios.post('http://nemonode.ivistaz.co/searchspl', searchData,{headers:{'Authorization':token}})
    ...
    async function handleDelete(candidateId) {
        ...
        if (confirmDelete) {
            try {
                ...
                await axios.delete(`http://nemonode.ivistaz.co/candidate/delete-candidate/${candidateId}`, { headers: { "Authorization": token } });
...
async function fetchCandidateData(candidateIds) {
  try {
    ...
    if (candidateIds && candidateIds.length > 0) {
      const response = await axios.get(`http://nemonode.ivistaz.co/candidate/get-candidate/${candidateIds}`, { headers: { "Authorization": token } });
...
const displayVesselTypeDropdown = async function () {
    try {
        ...
        const vesselResponse = await axios.get("http://nemonode.ivistaz.co/others/view-vsl", { headers: { "Authorization": token } });
...
const displayCountryDropdown = async function () {
    try {
        ...
        const countryResponse = await axios.get("http://nemonode.ivistaz.co/others/country-codes", { headers: { "Authorization": token } });
