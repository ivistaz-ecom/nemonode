...
async function displayGrade(page = 1, limit = 10) {
    try {
        ...
        const gradeResponse = await axios.get(`http://nemonode.ivistaz.co:8000/others/view-grade?page=${page}&limit=${limit}`, { headers: { "Authorization": token } });
...
async function deleteGrade(gradeId, event) {
    ...
    const id = gradeId;
    const url = `http://nemonode.ivistaz.co:8000/others/delete-grade/${id}`;
