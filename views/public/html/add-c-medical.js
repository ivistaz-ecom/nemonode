...
if (hasUserManagement && decodedToken.userGroup !== 'vendor') {
    ...
    document.getElementById('userManagementSections').style.display = 'block';
        ...
        try {
            const response = await axios.get(`${config.APIURL}candidate/get-hospital-details/${id}`, {
                headers: {
                    'Authorization': token,
                },
            });
    ...
    async function updateHospitalTable(data) {
        ...
        data.forEach(hospital => {
            ...
            row.innerHTML = `
                ...
                <td><a href='${config.APIURL}views/public/uploads/medical/${hospital.upload}' target="_blank">Click here to view Document!</a></td>
    ...
    const memId = urlParams.get('memId');
      ...
      document.getElementById('medicalForm').addEventListener('submit', async function (event) {
        ...
        try {
            const response = await axios.post(`${config.APIURL}candidate/hospital-details/${memId}`, medicalDetails, {
                headers: {
                    'Authorization': token,
                }
            });
