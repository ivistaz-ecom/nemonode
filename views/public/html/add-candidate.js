...
addcandidateButton.addEventListener("submit", async (e) => {
    ...
    try {
        ...
        if (photoFile) {
            photoFileName = await uploadFile(photoFile, 'http://localhost:4000/upload1');
        }

        if (resumeFile) {
            resumeFileName = await uploadFile(resumeFile, 'http://localhost:4000/upload3');
        }
        ...
        const serverResponse = await axios.post("http://localhost:4000/candidate/add-candidate", candidate_details, {
            headers: {
                "Authorization": token
            }
        });
