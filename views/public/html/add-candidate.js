...
async function  fetchAndDisplayExp() {
    try {
        const serverResponse = await axios.get(`${config.APIURL}others/view-experience`, { headers: { "Authorization": token } });
...
async function fetchAndDisplayGrades() {
    try {
        const serverResponse = await axios.get(`${config.APIURL}others/get-grade-drop`, { headers: { "Authorization": token } });
...
async function fetchAndDisplayNationalities() {
    try {
        const token = localStorage.getItem('token');
        const response = await axios.get(`${config.APIURL}fetch-nationality`, { headers: { "Authorization": token } });
...
addcandidateButton.addEventListener("submit", async (e) => {
    ...
    try {
        ...
        const serverResponse = await axios.post(`${config.APIURL}candidate/add-candidate`, candidate_details, {
            headers: {
                "Authorization": token
            }
        });
