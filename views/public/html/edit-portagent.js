...
async function displayPortagent(page = 1, limit = 10) {
    try {
        ...
        const portAgentResponse = await axios.get(`http://nemonode.ivistaz.co/others/view-port-agent?page=${page}&limit=${limit}`, { headers: { "Authorization": token } });
...
async function deletePortagent(portAgentId, event) {
    ...
    const url = `http://nemonode.ivistaz.co/others/delete-port-agent/${id}`;
