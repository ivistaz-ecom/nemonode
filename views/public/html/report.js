...
function exportToExcelnp(data, filename) {
    //console.log(selectedFields, selectedFieldsName, 'selectedFields')
  // Convert data to array of arrays for XLSX conversion
  const dataArray = data.map((candidate, index) => {
    const row = [];
    row.push(parseInt(index) + 1)
  ...
  // Insert headers
  let headers = Object.keys(selectedFields).filter((field) => selectedFields[field] === true).map((field) => selectedFieldsName[field]);

  headers.unshift('S.No');
  dataArray.unshift(headers);
  dataArray.unshift(['To Date', document.getElementById("endDate").value]);
  dataArray.unshift(['Form Date', document.getElementById("startDate").value]);
  dataArray.unshift(['New Profile']);
  dataArray.push([`----------Total New Profile ${dataArray.length}-----------`])
...
function displayTable(page=1) {
  ...
  for (const field in selectedFields) {
    if (selectedFields[field]) {
      ...
      th.textContent = selectedFieldsName[field];
  ...
  loadPagenation("pagination",page, totalPages, filteredCandidates.length, 'newprofile')
}

async function loadPageData(page, tableType) {
    if(tableType==='newprofile') {
        displayTable(page);
    }
...
function searchTable() {
  ...
  currentPage_ = 1; // Reset to the first page on new search
...
async function handleCallsMadeSubmit(event) {
  ...
  try {
    ...
    let currentPage_ = 1;
    let rowsPerPage = parseInt(
      document.getElementById("rowsPerPagecm")?.value || 10
    );
      ...
      return callsMade.filter((contract) => {
        return searchTerm.every((term) => {
          ...
          return selectedFields.some((field) => {
            const fieldName =
              fieldMapping[field.id] || field.id.replace("-", "_");
    ...
    function renderTable() {
      ...
      const start = (currentPage_ - 1) * rowsPerPage;
      const end = start + rowsPerPage;
      ...
      paginatedData.forEach((contract, index) => {
        ...
        selectedFields.forEach((field) => {
          ...
          if (fieldName === "nationality") {
            ...
            candidateIdButton.classList.add(
              "btn",
              "text-primary",
              "p-0",
              "ps-2",
            ...
            candidateIdButton.addEventListener("click", function () {
              window.open(
                `./view-candidate.html?id=${contract[fieldName]}`,
                "_blank"
              );
            });
            ...
            cell.textContent =
              contract[fieldName] === "1970-01-01"
                ? ""
                : contract[fieldName] || "N/A";
          }
      ...
      let startPage = currentPage_ - Math.floor(maxVisiblePages / 2);
  ...
  const worksheet = XLSX.utils.json_to_sheet(
    data.map((call, index) => {
      ...
      if (candidateData.nationality) {
        candidateData.nationality = getNationalityName(
      ...
      Object.keys(candidateData).forEach((key) => {
        if (
          candidateData[key] === "1970-01-01" ||
          candidateData[key] === "01-01-1970"
        ) {
...
async function handleSignOnSubmit(event) {
  ...
  try {
    ...
    function renderTable() {
      ...
      const headersWithNbsp = headers.map((headerText) =>
...
async function handleSignOffSubmit(event) {
  ...
  try {
    ...
    const signOffSearchContainer = document.getElementById(
      "signOffSearchContainer"
    );
    if (signOffSearchContainer) {
      ...
      document
        .querySelector(".container.table-responsive")
        .insertBefore(
          newSearchContainer,
          document.getElementById("signOffTable")
        );
    }
    ...
    const exportContainerSignOff = document.getElementById(
      "exportContainerSignOff"
    );
    const exportButton = document.createElement("button");
    exportButton.classList.add("btn", "btn-success", "p-0", "ps-2", "pe-2");
    exportButton.textContent = "Export to Excel";
  ...
  try {
    ...
    function renderPagination() {
      ...
      if (totalPages <= 5) {
        ...
        let startPage = Math.max(1, currentPage_ - 1);
        let endPage = Math.min(totalPages, currentPage_ + 1);

        if (currentPage_ === 1) {
          endPage = 3;
        } else if (currentPage_ === totalPages) {
...
async function handleDueForRenewalSubmit(event) {
  ...
  try {
    ...
    // Send data to server using Axios
    const response = await axios.get(
      `${config.APIURL}candidate/reports/renewal`,
      {
        params: params,
      }
    );
    ...
    const documentTableBody = document.getElementById(
      "documentCandidatesTableBody"
    );
    const medicalTableBody = document.getElementById(
      "medicalCandidatesTableBody"
    );
    ...
    documentCandidates.forEach((candidate) => {
      documentTableBody.appendChild(
        createDocumentRow(candidate, documentFields)
      );
    });
    ...
    if (!reports) {
      document.getElementById("exportDocumentCandidates").style.display =
        "none";
...
function setupPagination(contracts) {
  ...
  function filterContracts() {
    ...
    currentPage_ = 1; // Reset to first page after filtering
    displayTableRows();
            ...
            <tr>
                ...
                <td style='font-size:10px'>
                    <button onclick="viewCandidate(${
                      contract.candidateId
                    })" class="btn btn-link">${contract.candidateId}</button>
                </td>
                ...
                <td style='font-size:10px'>${
                  ...
                  calculateAge(contract.dob) === "01-01-1970"
                    ? ""
                ...
                <td style='font-size:10px'>${
                  formatDateNew(contract.eoc) === "1970-01-01" ||
                  formatDateNew(contract.eoc) === "01-01-1970"
                    ? ""
                    : formatDateNew(contract.eoc)
                }</td>
                <td style='font-size:10px'>${
                  formatDateNew(contract.sign_on) === "1970-01-01" ||
                  formatDateNew(contract.sign_on) === "01-01-1970"
                    ? ""
                    : formatDateNew(contract.sign_on)
                }</td>
                <td style='font-size:10px'>${
                  formatDateNew(contract.sign_off) === "1970-01-01" ||
                  formatDateNew(contract.sign_off) === "01-01-1970"
                    ? ""
                    : formatDateNew(contract.sign_off)
                }</td>
                <td style='font-size:10px'>${getPortName(
  ...
  function updatePaginationControls() {
    ...
    let startPage = Math.max(1, currentPage_ - Math.floor(maxPageButtons / 2));
...
async function handleCrewList(event) {
  ...
  try {
    ...
    exportButton.textContent = "Export to Excel";
    exportButton.addEventListener("click", () =>
      exportToExcel(filteredCrewlist)
    );
...
const handleReliefPlan = async (event) => {
  ...
  function displayReliefPlanTable(reliefPlanData) {
    ...
    function updatePagination(filteredData) {
      ...
      prevButton.disabled = currentPage_ === 1;
      ...
      for (let i = 1; i <= totalPages; i++) {
        const pageButton = document.createElement("button");
        pageButton.textContent = i;
        pageButton.disabled = i === currentPage_;
        pageButton.addEventListener("click", () => {
          currentPage_ = i;
          updateTable();
        });
      ...
      nextButton.disabled = currentPage_ === totalPages;
  ...
  function addExportButton(reliefPlanData) {
    ...
    exportButton.addEventListener("click", () =>
      exportReliefPlanToExcel(reliefPlanData)
    );
...
document
  .getElementById("exportDocumentCandidates")
  .addEventListener("click", function () {
    exportToExcels("documentCandidatesTableBody", "DocumentCandidates");
  });

document
  .getElementById("exportMedicalCandidates")
  .addEventListener("click", function () {
...
document.getElementById("getData").addEventListener("click", async () => {
  ...
  try {
    const response = await axios.get(`${config.APIURL}candidate/mis`, {
      params: {
        startDate,
        endDate,
      },
...
async function fetchCompanyName(companyId) {
  try {
    ...
    const response = await axios.get(
      `${config.APIURL}company/get-company/${companyId}`,
      { headers: { Authorization: token } }
    );
...
async function handleDueForWorkedWithSubmit(event, pages = 1) {
  ...
  try {
    ...
    const response = await axios.get(`${config.APIURL}candidate/worked`, {
      params: {
    ...
    // Clear existing tables, if any
    const candidatesTableContainer = document.getElementById(
      "CandidatesTableBody"
    );
    candidatesTableContainer.innerHTML = "";

    const contractsTableContainer =
    ...
    onboardContracts.forEach((contract) => {
      ...
      const fields = [contract.sign_on, contract.sign_off];
      fields.forEach((field) => {
        const cell = document.createElement("td");
        cell.textContent = field;
        cell.classList.add("text-center");
...
document
  .getElementById("dueForWorkedWithForm")
...
function updatePaginationInfo() {
  document.getElementById("totalPagesCandidates").textContent =
    "Page " + pagenumber + " of " + totalPagesCandidates;
  document.getElementById("totalPagesContracts").textContent =
    "Page " + pagenumber + " of " + totalPagesContracts;
}
...
function getCompanyName(code) {
  const companies = companiesData.find(
    (companies) => companies.company_id == code
  );
