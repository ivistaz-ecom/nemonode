...
async function displayExperiences(page = 1, limit = 10) {
    try {
        ...
        const expResponse = await axios.get(`http://nemonode.ivistaz.co:8000/others/view-experience?page=${page}&limit=${limit}`, { headers: { "Authorization": token } });
...
async function deleteExperience(expId, event) {
    ...
    const id = expId;
    const url = `http://nemonode.ivistaz.co:8000/others/delete-experience/${id}`;
